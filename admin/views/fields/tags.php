...
<div class="tag-input<?php if ($field->get('disabled')): ?> disabled<?php endif; ?>">
    ...
    <input class="tag-hidden-input" name="<?= $field->name() ?>" hidden readonly value="<?= implode(', ', (array) $field->value()) ?>"<?php if ($field->get('required')): ?> required<?php endif; ?><?php if ($field->get('disabled')): ?> disabled<?php endif; ?>>
</div>
