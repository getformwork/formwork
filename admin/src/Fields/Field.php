...
class Field extends DataSetter {
    ...
    public function __construct($name, $data = array(), Fields $parent = null) {
        ...
        if ($this->has('import')) $this->importData();
        if ($this->has('fields')) {
    ...
    protected function importData() {
        foreach ((array) $this->data['import'] as $key => $value) {
            if ($key == 'import') throw new Exception('Invalid key for import');
            $callback = explode('::', $value, 2);
            if (!is_callable($callback)) throw new Exception('Invalid import callback');
    ...
    public function get($key, $default = null) {
        ...
        if ($this->translatable($key) && is_array($value)) {
            if (isset($value[$this->language])) return $value[$this->language];
        }
        return $value;
    }
    ...
    public function translatable($key) {
        ...
        if (is_array($translate)) return in_array($key, $translate);
        return $translate;
    }
    ...
    public function __debugInfo() {
        ...
        if ($this->has('type')) $return['type'] = $this->get('type');
        if ($this->has('default')) $return['default'] = $this->get('default');
        if ($this->has('value')) $return['value'] = $this->get('value');
        if ($this->has('fields')) $return['fields'] = $this->get('fields');
        return $return;
    }
