...
class Uploader {
    ...
    public function upload($name = null) {
        if (!HTTPRequest::hasFiles()) return false;
        $count = count(HTTPRequest::files());

        foreach (HTTPRequest::files() as $file) {
            if ($file['error'] === 0) {
                if (is_null($name) || $count > 1) $name = $file['name'];
    ...
    public function isAllowedMimeType($mimeType) {
        if (is_null($this->options['allowedMimeTypes'])) return true;
    ...
    private function __move($source, $destination, $filename) {
        ...
        if (!$this->options['overwrite'] && FileSystem::exists($destination . $filename)) return false;
